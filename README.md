...
- **在线配置**: 通过网页直接添加/删除监控源、修改转发规则、编辑黑白名单。
- **热重载**: 修改配置后，通过 Web 或 Bot 指令即可热重载，无需重启容器。
- **AI 判别**: 开关 AI 广告判别（F13）并在线调参（端点/模型/阈值/模糊区/超时），默认关，保存即热重载生效。
- **AI 处理**: F14 结构化内容处理——一次模型调用同时产出广告判定与清洗后正文（JSON 结构化输出）。是否丢弃由服务端按阈值决定，不采信模型自行拍板；清洗可单独关掉（只判广告不改文），保存即热重载生效。
- **版本号**: 系统设置页显示当前版本（单一事实源 = git tag，构建期注入镜像）。
- **安全验证**: 内置登录验证，保护你的配置信息。
